...
# Diese Funktion misst die Ausführungszeit einer übergebenen Funktion.
# Sie verwendet das Decorator-Pattern, um die Funktionalität der übergebenen Funktion zu erweitern.
# Der Wrapper führt die übergebene Funktion aus, misst die Zeit vor und nach der Ausführung
# und gibt die Differenz als Ausgabe aus.
def zeit_messen(funktion):
    def wrapper(*args, **kwargs):
        startzeit = time.time()  # Startzeit messen
        ergebnis = funktion(*args, **kwargs)  # Übergebene Funktion ausführen
        endzeit = time.time()  # Endzeit messen
        print(f"Zeit für {funktion.__name__}: {endzeit - startzeit} Sekunden")  # Ausführungszeit ausgeben
        return ergebnis
    return wrapper
